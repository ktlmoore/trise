...
public class DecideByAStarSearch implements DecisionFunction {
	 ...
	 */
	public DecideByAStarSearch(GoalFunction goal,
	...
	/**
	 * Applies the decision function on the environment to get the decision
	 * graph and the action the agent should take
	 * 
	 * @param env
	 */
	@Override
	public Tuple<MutableMetrics, Tuple<Action, TrackedGraph<Vector2>>> apply(
		...
		// First we check to see if we've even got a decision graph. If not, we
		// make sure we do.
		if (!initialised) {
			...
			System.out.println(String.format(
					"Skeletonised environment in %dms",
					metrics.getTimeToSkeletonise()));

			initialised = true;
		}

		// If the path we're following has no more steps, we look for a new path
		// to follow
		if (path.size() <= 0 && !atGoal) {

			// Reset search tools
			frontier = new PriorityQueue<>(
					(x, y) -> (int) (heuristicFunction.apply(x) - heuristicFunction
							.apply(y)));
			explored = new HashSet<Node<Vector2>>();
			pathBack = new HashMap<Node<Vector2>, Node<Vector2>>();
			...
			if (goal.apply(t, startNode)) {
				...
				// If we're not at a goal, we continue and put the start node in
				// the frontier.
				// It is also part of the path back to the start, for obvious
				// reasons.
				...
				// We now keep looking through the frontier until it is empty or
				...
				System.out.println(String.format("Explored %d nodes",
						nodesExplored));
				System.out.println(String.format(
						"Put %d nodes in the frontier", nodesInFrontier));
				...
				// Add the goal node to the end of the path, as this will be our
				// final node.
				path.add(goalNode);
				...
				// Now we start calculating our path back from the goal to the
				// start
		...
		if (!atGoal) {
			...
			// And then we move to that location, mark it as visited and return
			// the data. Simple.
